...
workpath='D:\\maticServer\\factor_matic_server\\pattern_analysis_py\\'
...
stdin_handler = open(workpath + 'std\\stdin','w')
stdout_handler = open(workpath + 'std\\stdout','w')
stderr_handler= open(workpath + 'std\\stderr','w')
...
#读取收盘价
f=open("D:\\maticServer\\mdc_pattern_analysis\\target\\mdc\\bin\\marketValue.txt","r")
data=f.read().split("\n")
f.close()
#去0处理
new_data=[]
...
#	print(price)
	...
	# print(one_info)
	# print(Closing_price)
	# if float(abs(result["maxampTop"]))/Closing_price>0.1:
	# 	Outliers.append(one_info)
# print(info)
...
stop_time=time.time()
logging.info('Task is done,cosumes :%d'%(stop_time-start_time))
