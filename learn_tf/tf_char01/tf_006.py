...
def train(mnist):
    ...
    # 为啥一次都写了  weights不是第二层的损失吗?  不应该是每层的权重进行L2正则吗?
    ...
    learning_rate = tf.train.exponential_decay\
        (LEARNING_RATE_BASE, global_step, mnist.train.num_examples/BATCH_SIZE, LEARNING_RETE_DECAY)
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            if i % 1000 == 0:
                ...
                print('训练{}次后, 精确度为:{}'.format(i, validate_acc))

            xs, ys = mnist.train.next_batch(BATCH_SIZE)
            sess.run(train_op, feed_dict={x: xs, y_: ys})

        test_acc = sess.run(accuracy, feed_dict=test_feed)
        print('测试集上, 精确度为:{}'.format(test_acc))
